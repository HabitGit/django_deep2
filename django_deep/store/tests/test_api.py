...
class BooksApiTestCase(APITestCase):
    ...
    def test_get_ordering_alphabet(self):
        url = reverse('book-list')
        books = Book.objects.all().annotate(
            annotated_likes=Count(Case(When(userbookrelation__like=True, then=1))),
            rating=Avg('userbookrelation__rate')
    ...
    def test_delete_not_owner(self):
        ...
        response = self.client.delete(url)
        serializer_data = BooksSerializer(books, many=True).data
        response2 = self.client.get(url_check)

        self.assertEqual(status.HTTP_403_FORBIDDEN, response.status_code)
        self.assertEqual({'detail': ErrorDetail(string='You do not have permission to perform this action.',
                                                code='permission_denied')}, response.data)
        self.assertEqual(serializer_data, response2.data)
    ...
    def test_delete_not_owner_but_staff(self):
        ...
        url = reverse('book-detail', args=(self.book_1.id,))
        url_check = reverse('book-list')
        books = Book.objects.filter(id__in=[self.book_2.id,
                                           self.book_3.id]).annotate(
            annotated_likes=Count(Case(When(userbookrelation__like=True, then=1))),
            rating=Avg('userbookrelation__rate')
        ).order_by('id')
